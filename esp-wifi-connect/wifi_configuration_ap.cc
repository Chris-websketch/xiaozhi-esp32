...
void WifiConfigurationAp::StartWebServer()
{
    ...
    config.max_uri_handlers = 30;  // 增加处理器数量以支持更多Captive Portal端点
    ...
    httpd_uri_t index_html = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ESP_LOGI(TAG, "Root path accessed from client");
            
            // 获取Host和User-Agent（用于调试）
            char host[128] = {0};
            char user_agent[256] = {0};
            httpd_req_get_hdr_value_str(req, "Host", host, sizeof(host));
            httpd_req_get_hdr_value_str(req, "User-Agent", user_agent, sizeof(user_agent));
            ESP_LOGI(TAG, "Host: %s, User-Agent: %s", host, user_agent);
    ...
    // iOS专用检测端点处理器 - 直接重定向到配网页面
    auto ios_detect_handler = [](httpd_req_t *req) -> esp_err_t {
        auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
        ...
        if (httpd_req_get_hdr_value_str(req, "Host", host, sizeof(host)) == ESP_OK) {
            ESP_LOGI(TAG, "Host header: %s", host);
        }
        
        // 直接返回302重定向到配网页面（使用实际IP地址）
        std::string redirect_url = this_->GetWebServerUrl() + "/?lang=" + this_->language_;
        httpd_resp_set_type(req, "text/html");
        httpd_resp_set_status(req, "302 Found");
        httpd_resp_set_hdr(req, "Location", redirect_url.c_str());
        httpd_resp_set_hdr(req, "Cache-Control", "no-cache, no-store, must-revalidate");
        httpd_resp_send(req, NULL, 0);
        ESP_LOGI(TAG, "iOS detection redirect sent to: %s", redirect_url.c_str());
        return ESP_OK;
    };

    // Android专用检测端点处理器 - 返回非204状态码
    auto android_detect_handler = [](httpd_req_t *req) -> esp_err_t {
        // Android期望204 No Content，返回200会触发Captive Portal
    ...
    // Register iOS captive portal detection endpoints
    const char* ios_detect_urls[] = {
        "/hotspot-detect.html",         // iOS主要检测端点
        "/library/test/success.html",   // iOS备用检测端点
    };
    for (const auto& url : ios_detect_urls) {
        httpd_uri_t ios_uri = {
            .uri = url,
            .method = HTTP_GET,
            .handler = ios_detect_handler,
            .user_ctx = this
        };
        ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &ios_uri));
    }

    // Register Android captive portal detection endpoints
    const char* android_detect_urls[] = {
        "/generate_204",           // Android主要检测端点
        "/gen_204",                // Android备用端点
    };
    for (const auto& url : android_detect_urls) {
        httpd_uri_t android_uri = {
            .uri = url,
            .method = HTTP_GET,
            .handler = android_detect_handler,
            .user_ctx = this
        };
        ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &android_uri));
    }

    // Register other captive portal detection endpoints
    const char* other_captive_urls[] = {
        "/mobile/status.php",      // Android旧版本
    ...
    for (const auto& url : other_captive_urls) {
    ...
    httpd_uri_t wildcard_uri = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            httpd_resp_set_hdr(req, "Location", url.c_str());
            httpd_resp_send(req, NULL, 0);
            ESP_LOGI(TAG, "Wildcard redirect from: %s", req->uri);
            return ESP_OK;
        },
        .user_ctx = this
    };
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &wildcard_uri));

    ESP_LOGI(TAG, "Web server started with optimized Captive Portal detection");
}
