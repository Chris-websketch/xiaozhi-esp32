...
- 字段说明：
  ...
  - title: 可选，通知标题（字符串）
  - body: 可选，通知正文（字符串）
  - duration_ms: 可选，显示时长（毫秒），范围 1000-300000，默认 60000（60秒）
- 设备行为：
  - 设备将把 title 与 body 组合为一条消息（若两者同时存在，以换行分隔），在屏幕上显示指定时长
  - 若仅提供其一，则显示提供的字段内容；若二者均缺省，则忽略该消息
  - 用户可通过单击 Boot 按键提前关闭通知
...
示例（自定义显示5秒）：
```json
{"type":"notify","title":"标题","body":"内容","duration_ms":5000}
```

示例（显示3分钟）：
```json
{"type":"notify","title":"重要通知","body":"这是一条需要长时间显示的通知","duration_ms":180000}
```
